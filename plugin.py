...
class MHWJHAction(BaseAction):
    ...
    # === 功能描述（必须填写）===
    action_parameters = {
        "MHW_jhm_action":"当前需要执行的集会码指令，必须。请注意选择当前需要执行的指令，填写的为冒号后面的英文。登记集会码：REG，删除集会码：DEL，查询集会码：QUE",
        "REG_jhm":"需要登记的集会码，可选。注意！！集会码只能包含英文、除了逗号句号以外的标点符号和数字，不可以包含中文",
...
class MHWorldPlugin(BasePlugin):
    ...
    def get_plugin_components(self) -> List[Tuple[ComponentInfo, Type]]:
        """返回插件包含的组件列表"""
        return [
            (MHWJHAction.get_action_info(), MHWJHAction)
        ]
